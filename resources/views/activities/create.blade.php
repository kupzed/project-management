...
<div class="flex min-h-full flex-col justify-center px-6 py-12 lg:px-8">
    ...
    <div class="mt-10 sm:mx-auto sm:w-full sm:max-w-sm">
        <form class="space-y-6" action="{{ route('activities.store') }}" method="POST" enctype="multipart/form-data">
            @csrf
            ...
            <div>
                ...
                <div class="mt-2">
                    <select id="kategori" name="kategori" required class="block w-full rounded-md bg-white px-3 py-1.5 text-base text-gray-900 outline-1 -outline-offset-1 outline-gray-300 placeholder:text-gray-400 focus:outline-2 focus:-outline-offset-2 focus:outline-indigo-600 sm:text-sm">
                        ...
                        <option value="Expense Report" {{ old('kategori') == 'Expense Report' ? 'selected' : '' }}>Expense Report</option>
                        <option value="Invoice" {{ old('kategori') == 'Invoice' ? 'selected' : '' }}>Invoice</option>
                        <option value="Purchase Order" {{ old('kategori') == 'Purchase Order' ? 'selected' : '' }}>Purchase Order</option>
                        <option value="Payment" {{ old('kategori') == 'Payment' ? 'selected' : '' }}>Payment</option>
                        <option value="Quotation" {{ old('kategori') == 'Quotation' ? 'selected' : '' }}>Quotation</option>
                        <option value="Faktur Pajak" {{ old('kategori') == 'Faktur Pajak' ? 'selected' : '' }}>Faktur Pajak</option>
                        <option value="Kasbon" {{ old('kategori') == 'Kasbon' ? 'selected' : '' }}>Kasbon</option>
                        <option value="Laporan Teknis" {{ old('kategori') == 'Laporan Teknis' ? 'selected' : '' }}>Laporan Teknis</option>
            ...
            <div>
                ...
                <div class="mt-2">
                    <input type="date" name="activity_date" id="activity_date" value="{{ old('activity_date') }}" required class="block w-full rounded-md bg-white px-3 py-1.5 text-base text-gray-900 outline-1 -outline-offset-1 outline-gray-300 placeholder:text-gray-400 focus:outline-2 focus:-outline-offset-2 focus:outline-indigo-600 sm:text-sm" />
                </div>
                @error('activity_date')
                    <p class="mt-1 text-sm text-red-600">{{ $message }}</p>
                @enderror
            ...
            <div class="col-span-full">
                ...
                <div class="mt-2 flex justify-center rounded-lg border border-dashed border-gray-900/25 px-6 py-10">
                    <div class="text-center">
                        ...
                        <p id="file-name" class="text-xs/5 text-gray-600"></p>
