...
<Modal bind:show={show} {title} maxWidth="max-w-xl" {onClose}>
  <form onsubmit={handleFormSubmit} autocomplete="off">
    <fieldset disabled={isSubmitting} class="space-y-4">
      <div>
        <label for={`${idPrefix}_name`} class="block text-sm font-medium text-gray-900 dark:text-white">Nama</label>
        <input id={`${idPrefix}_name`} type="text" bind:value={form.name} required placeholder="Masukkan nama sertifikat" class="mt-1 block w-full rounded-md border border-gray-300 bg-white px-3 py-2 text-sm text-gray-900 focus:ring-1 focus:ring-indigo-500 focus:outline-none dark:border-gray-700 dark:bg-neutral-900 dark:text-gray-100" />
      </div>

      <div>
        <label for={`${idPrefix}_no_certificate`} class="block text-sm font-medium text-gray-900 dark:text-white">No Sertifikat</label>
        <input id={`${idPrefix}_no_certificate`} type="text" bind:value={form.no_certificate} required placeholder="Masukkan nomor sertifikat" class="mt-1 block w-full rounded-md border border-gray-300 bg-white px-3 py-2 text-sm text-gray-900 focus:ring-1 focus:ring-indigo-500 focus:outline-none dark:border-gray-700 dark:bg-neutral-900 dark:text-gray-100" />
      </div>

      {#if showProjectSelect}
        <div>
          <label for={`${idPrefix}_project`} class="block text-sm font-medium text-gray-900 dark:text-white">Project</label>
          <select id={`${idPrefix}_project`} bind:value={form.project_id} required onchange={handleProjectSelectChange} class="mt-1 block w-full rounded-md border border-gray-300 bg-white px-3 py-2 text-sm text-gray-900 focus:ring-1 focus:ring-indigo-500 focus:outline-none dark:border-gray-700 dark:bg-neutral-900 dark:text-gray-100">
            <option value="">Pilih Project</option>
            {#each projects as project (project.id)}<option value={project.id}>{project.name ?? project.title}</option>{/each}
          </select>
        </div>
      {/if}
      ...
      <div>
        ...
        <select id={`${idPrefix}_barang_certificate`} bind:value={form.barang_certificate_id} required disabled={barangOptions.length === 0} class="mt-1 block w-full rounded-md border border-gray-300 bg-white px-3 py-2 text-sm text-gray-900 focus:ring-1 focus:ring-indigo-500 focus:outline-none dark:border-gray-700 dark:bg-neutral-900 dark:text-gray-100">
          ...
          {#each barangOptions as barang (barang.id)}
            <option value={barang.id}>{barang.name ?? barang.title} {#if barang.no_seri}- {barang.no_seri}{/if}</option>
          {/each}
        </select>
      ...
      <div>
        ...
        <select id={`${idPrefix}_status`} bind:value={form.status} required class="mt-1 block w-full rounded-md border border-gray-300 bg-white px-3 py-2 text-sm text-gray-900 focus:ring-1 focus:ring-indigo-500 focus:outline-none dark:border-gray-700 dark:bg-neutral-900 dark:text-gray-100">
      ...
      <div class="grid grid-cols-1 gap-4 md:grid-cols-2">
        <div>
          <label for={`${idPrefix}_date_of_issue`} class="block text-sm font-medium text-gray-900 dark:text-white">Tanggal Terbit</label>
          <input id={`${idPrefix}_date_of_issue`} type="date" bind:value={form.date_of_issue} class="mt-1 block w-full rounded-md border border-gray-300 bg-white px-3 py-2 text-sm text-gray-900 focus:ring-1 focus:ring-indigo-500 focus:outline-none dark:border-gray-700 dark:bg-neutral-900 dark:text-gray-100" />
        </div>
        <div>
          <label for={`${idPrefix}_date_of_expired`} class="block text-sm font-medium text-gray-900 dark:text-white">Tanggal Expired</label>
          <input id={`${idPrefix}_date_of_expired`} type="date" bind:value={form.date_of_expired} class="mt-1 block w-full rounded-md border border-gray-300 bg-white px-3 py-2 text-sm text-gray-900 focus:ring-1 focus:ring-indigo-500 focus:outline-none dark:border-gray-700 dark:bg-neutral-900 dark:text-gray-100" />
        </div>
      </div>
    ...
    <div class="mt-6">
      <button type="submit" disabled={isSubmitting} aria-busy={isSubmitting} class="flex w-full items-center justify-center gap-2 rounded-md bg-indigo-600 px-3 py-1.5 text-sm/6 font-semibold text-white shadow-xs hover:bg-indigo-500 focus-visible:outline-2 focus-visible:outline-offset-2 focus-visible:outline-indigo-600 disabled:cursor-not-allowed disabled:opacity-60">
        {#if isSubmitting}
