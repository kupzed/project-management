...
## 🛠️ Rekomendasi Tools API Documentation Generator

### 1. **Scribe** (⭐ Direkomendasikan untuk Laravel)
...
**Kelebihan:**
- Otomatis men-scan route, FormRequest, dan Resource
- Menghasilkan HTML docs yang interaktif & Postman collection
- Mendukung anotasi `@bodyParam`, `@response`, dll
- Dokumentasi di-generate di `/docs`
 ...
 * Buat Aktivitas Baru
 *
 * Membuat record aktivitas baru pada sebuah proyek.
 * Mendukung upload multi-attachment dan AI Document Extraction.
 *
 * @group Activities
 * @authenticated
 *
 * @bodyParam name string required Nama aktivitas. Example: Invoice Pembelian
 * @bodyParam project_id int required ID proyek. Example: 3
 * @bodyParam jenis string required Jenis aktivitas. Example: Vendor
 *
 * @response 201 scenario="Success" {"message":"Activity created successfully","data":{...}}
 * @response 422 scenario="Validation Error" {"message":"The given data was invalid.","errors":{...}}
 */
public function store(ActivityRequest $request) { ... }
```

### 2. **L5-Swagger** (OpenAPI/Swagger)
...
**Kelebihan:**
- Menghasilkan Swagger UI di `/api/documentation`
- Standar industri (OpenAPI 3.0)
- Cocok jika API akan dikonsumsi oleh tim eksternal

### 3. **Scramble** (Laravel API Docs yang Otomatis)
...
**Kelebihan:**
- Zero-config — otomatis membaca route dan tipe PHP
- Tidak perlu anotasi manual
- UI modern dan responsif di `/docs/api`

> [!TIP]
> Untuk proyek ini, **Scribe** adalah pilihan terbaik karena menghasilkan dokumentasi yang detail tanpa terlalu banyak setup, dan mendukung export ke format Postman Collection untuk kemudahan testing.
